...
class Query<T> {
   ...
   */
  public fetchData({ queryParams, params = {} }: IFetchDataProps<T>): UseQueryResult<T | T[], Error> {
    return useQuery<T | T[], Error>({
      queryKey: [this.proxy, this.key],
      queryFn: () =>
        this.ClassModule
          ? getService(this.proxy, this.url, this.ClassModule, params)
          : getService(this.proxy, this.url, params),
      ...queryParams,
    });
  }
